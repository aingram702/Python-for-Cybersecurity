...
def createFakeCookie(name,value,host,path):
    ...
    dt = datetime.now()
    lastAccessed = time.mktime(dt.timetuple())*le6+dt.microsecond
    creationTime = time.mktime(dt.timetuple())*le6+dt.microsecond
    query = "INSERT INTO mox_cookies ('name','value','host','path',\
    'expiry','lastAccessed','creationTime','isSecure','isHttpOnly',\
    'schemeMap') VALUES ('%s','%s','%s','%s','%d','%d','%d','%d',\
    '%d','%d');" % (name,value,host,path,expiry,lastAccessed,creationTime,0,0,2)
